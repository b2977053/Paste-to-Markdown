'use strict';

window.i18nLocales = window.i18nLocales || {};
window.i18nLocales.hi = {
  localeName: 'हिन्दी',
  brandSubtitle: 'रिच क्लिपबोर्ड सामग्री को स्थानीय रूप से Markdown में बदलें, फिर एक केंद्रित कार्यक्षेत्र में परिणाम संपादित करें या उसका पूर्वावलोकन करें।',
  subtitle: 'उपयोग कैसे करें',
  introSubtitle: 'जिस स्रोत का आप पहले से उपयोग करते हैं, वहीं से पेस्ट करें। रूपांतरण इसी डिवाइस पर होता है।',
  step1: 'रिच सामग्री को {copyShortcut} से कॉपी करें',
  step2: 'इस पेज पर {pasteShortcut} से पेस्ट करें',
  step3: 'संपादित करें, पूर्वावलोकन देखें, फिर Markdown परिणाम कॉपी करें',
  step4: 'रीसेट करने के लिए <code>Escape</code> दबाएं',
  step5: '{editShortcut} और {previewShortcut} से टैब बदलें',
  basedOn: 'यह कनवर्टर आधारित है',
  basedOnAfter: ' पर। सभी डेटा स्थानीय रूप से संसाधित किया जाता है, किसी भी सर्वर पर कुछ भी नहीं भेजा जाता है।',
  styleFrom: 'शैली यहाँ से अनुकूलित',
  styleFromAfter: '.',
  sourceCode: 'स्रोत कोड यहाँ उपलब्ध है',
  feedbackPre: ', ',
  issues: 'समस्याएं',
  feedbackPost: '.',
  tabEdit: 'संपादित करें',
  tabPreview: 'पूर्वावलोकन',
  workspaceStatus: 'पेस्ट करने के बाद कॉपी करने के लिए तैयार',
  themeSystem: 'सिस्टम',
  themeLight: 'हल्का',
  themeDark: 'गहरा',
  fullscreenButton: '⛶ पूर्ण स्क्रीन',
  fullscreenButtonTitle: 'एडिटर को पूर्ण स्क्रीन में फैलाएँ',
  fullscreenExit: '🗗 पूर्ण स्क्रीन से बाहर निकलें',
  fullscreenExitTitle: 'पूर्ण स्क्रीन मोड से बाहर निकलें',
  fullscreenUnsupported: 'यह ब्राउज़र पूर्ण स्क्रीन मोड का समर्थन नहीं करता',
  shareButton: '🔗 शेयर करें',
  shareButtonTitle: 'साझा करने योग्य URL कॉपी करें',
  shareSuccess: '🔗 लिंक कॉपी हो गया',
  copyButton: '📋 कॉपी करें',
  copyButtonTitle: 'Markdown और HTML के रूप में कॉपी करें',
  copySuccess: '📋 कॉपी हो गया',
  placeholder: 'सामग्री यहाँ पेस्ट करें...',
  noPreview: 'पूर्वावलोकन के लिए कोई सामग्री नहीं है'
};
