'use strict';

window.i18nLocales = window.i18nLocales || {};
window.i18nLocales.pt = {
  localeName: 'Português',
  brandSubtitle: 'Converta conteúdo rico da área de transferência para Markdown localmente e depois edite ou visualize o resultado em um espaço de trabalho focado.',
  subtitle: 'Como usar',
  introSubtitle: 'Cole a partir da fonte que você já usa. A conversão acontece neste dispositivo.',
  step1: 'Copie conteúdo rico com {copyShortcut}',
  step2: 'Cole nesta página com {pasteShortcut}',
  step3: 'Edite, visualize e copie o Markdown gerado',
  step4: 'Pressione <code>Escape</code> para reiniciar',
  step5: 'Alterne as abas com {editShortcut} e {previewShortcut}',
  basedOn: 'Este conversor é baseado em',
  basedOnAfter: '. Todos os dados são processados localmente, nada é enviado para nenhum servidor.',
  styleFrom: 'Estilo adaptado de',
  styleFromAfter: '.',
  sourceCode: 'Código fonte disponível em',
  feedbackPre: ', ',
  issues: 'Problemas',
  feedbackPost: '.',
  tabEdit: 'Editar',
  tabPreview: 'Visualizar',
  workspaceStatus: 'Pronto para copiar depois de colar',
  themeSystem: 'Sistema',
  themeLight: 'Claro',
  themeDark: 'Escuro',
  fullscreenButton: '⛶ Tela cheia',
  fullscreenButtonTitle: 'Expandir o editor para tela cheia',
  fullscreenExit: '🗗 Sair da tela cheia',
  fullscreenExitTitle: 'Sair do modo de tela cheia',
  fullscreenUnsupported: 'Este navegador não oferece suporte à tela cheia',
  shareButton: '🔗 Compartilhar',
  shareButtonTitle: 'Copiar URL compartilhável',
  shareSuccess: '🔗 Link copiado',
  copyButton: '📋 Copiar',
  copyButtonTitle: 'Copiar como Markdown e HTML',
  copySuccess: '📋 Copiado',
  placeholder: 'Cole o conteúdo aqui...',
  noPreview: 'Nenhum conteúdo para visualizar'
};
