'use strict';

window.i18nLocales = window.i18nLocales || {};
window.i18nLocales.ar = {
  localeName: 'العربية',
  brandSubtitle: 'حوّل محتوى الحافظة المنسّق إلى Markdown محليًا، ثم حرّره أو عاينه في مساحة عمل واحدة مركّزة.',
  subtitle: 'كيفية الاستخدام',
  introSubtitle: 'الصق من المصدر الذي تستخدمه بالفعل. تتم عملية التحويل على هذا الجهاز.',
  step1: 'انسخ المحتوى الغني باستخدام {copyShortcut}',
  step2: 'الصق في هذه الصفحة باستخدام {pasteShortcut}',
  step3: 'حرّر النتيجة وعاينها، ثم انسخ Markdown',
  step4: 'اضغط على <code>Escape</code> لإعادة التعيين',
  step5: 'بدّل التبويبات باستخدام {editShortcut} و {previewShortcut}',
  basedOn: 'هذا المحول مبني على',
  basedOnAfter: '. يتم معالجة جميع البيانات محليًا، ولا يتم إرسال أي شيء إلى أي خادم.',
  styleFrom: 'النمط مقتبس من',
  styleFromAfter: '.',
  sourceCode: 'كود المصدر متاح في',
  feedbackPre: '، ',
  issues: 'المشكلات',
  feedbackPost: '.',
  tabEdit: 'تحرير',
  tabPreview: 'معاينة',
  workspaceStatus: 'جاهز للنسخ بعد اللصق',
  themeSystem: 'النظام',
  themeLight: 'فاتح',
  themeDark: 'غامق',
  fullscreenButton: '⛶ ملء الشاشة',
  fullscreenButtonTitle: 'توسيع المحرر إلى وضع ملء الشاشة',
  fullscreenExit: '🗗 إنهاء ملء الشاشة',
  fullscreenExitTitle: 'الخروج من وضع ملء الشاشة',
  fullscreenUnsupported: 'هذا المتصفح لا يدعم وضع ملء الشاشة',
  shareButton: '🔗 مشاركة',
  shareButtonTitle: 'نسخ رابط المشاركة',
  shareSuccess: '🔗 تم نسخ الرابط',
  copyButton: '📋 نسخ',
  copyButtonTitle: 'نسخ كـ Markdown و HTML',
  copySuccess: '📋 تم النسخ',
  placeholder: 'الصق المحتوى هنا...',
  noPreview: 'لا يوجد محتوى للمعاينة'
};
